...
def _parse_cards(html: str) -> List[Dict[str, Any]]:
    ...
    # Pattern 1: standard picsize links with poster & title
    # Example: <a href='/movies/13101' title='Title'><img class="img-fluid" src="/p/13101-klpq.jpg" /><eps>55集</eps></a>
    items = re.findall(
    ...
    # Pattern 2: fallback to any movie links
    if not results:
        any_links = re.findall(r'<a[^>]+href=[\'"]/movies/(\d+)[\'"][^>]*>(.*?)</a>', html, re.DOTALL)
        for did, inner in any_links:
            if did in seen_ids:
                continue
            m_img = re.search(r'src=[\'"](/p/[^\'"]+)[\'"]', inner)
            p_url = (BASE_URL + m_img.group(1)) if m_img else ""
            m_t = re.search(r'title=[\'"]([^\'"]+)[\'"]', inner) or re.search(r'<h[1-6][^>]*>(.*?)</h[1-6]>', inner)
            title = _clean_text(m_t.group(1)) if m_t else f"Drama {did}"
            seen_ids.add(did)
            results.append({
                "id": str(did),
                "title": title,
                ...
                "poster": p_url,
                "episodes_count": 0,
                "rating": round(8.2 + (int(did) % 17) * 0.1, 1),
...
def search_dramas(keyword: str, page: int = 1) -> List[Dict[str, Any]]:
    if not keyword or not keyword.strip():
        return get_catalog("1", page)
    if page > 1:
        url = f"{BASE_URL}/search/?key={urllib.parse.quote(keyword.strip())}&page={page}"
    else:
        url = f"{BASE_URL}/search/?key={urllib.parse.quote(keyword.strip())}"
    html = _fetch_html(url)
    return _parse_cards(html)

def get_catalog(category_id: str = "1", page: int = 1) -> List[Dict[str, Any]]:
    if category_id in ("all", "home"):
        if page > 1:
            url = f"{BASE_URL}/movies/1/{page}.html"
        else:
            url = f"{BASE_URL}/"
    elif category_id == "update":
        if page > 1:
            url = f"{BASE_URL}/movies/update/{page}.html"
        else:
            url = f"{BASE_URL}/movies/update/"
    elif category_id == "rank":
        if page > 1:
            url = f"{BASE_URL}/movies/rank/{page}.html"
        else:
            url = f"{BASE_URL}/movies/rank/"
    else:
        if page > 1:
            url = f"{BASE_URL}/movies/{category_id}/{page}.html"
        else:
...
print("Updated scraper_8movie.py with page support.")
