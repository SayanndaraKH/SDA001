...
def _enrich_titles_with_khmer(cards: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if titles:
        trans_map = translate_batch(titles[:30])
        for c in cards:
...
@app.get("/api/catalog")
async def api_catalog(cat: str = Query("1", description="Category ID")):
    cards = get_catalog(cat)
    cards = _enrich_titles_with_khmer(cards)
    return {"category": cat, "count": len(cards), "items": cards}

@app.get("/api/search")
async def api_search(q: str = Query("", description="Search keyword")):
    cards = search_dramas(q)
    cards = _enrich_titles_with_khmer(cards)
    return {"query": q, "count": len(cards), "items": cards}

@app.get("/api/rank")
async def api_rank():
    cards = get_catalog("rank")
    cards = _enrich_titles_with_khmer(cards)
    return {"category": "rank", "count": len(cards), "items": cards}

@app.get("/api/latest")
async def api_latest():
    cards = get_catalog("update")
    cards = _enrich_titles_with_khmer(cards)
    return {"category": "update", "count": len(cards), "items": cards}
...
async def get_library():
    """Scan the download directory and list existing dramas, episodes, and posters."""
...
with open(os.path.join(TARGET_DIR, 'app', 'server.py'), 'w', encoding='utf-8') as f:
    f.write(server_code)

print("Written: app/server.py")
