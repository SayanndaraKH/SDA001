...
TARGET = r"C:\Users\Administrator\Desktop\SYD-8Move\app\web\index.html"
...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; margin:0; padding:0; }
    ...
    .btn.teal{
      background:linear-gradient(115deg,var(--teal),var(--teal-2)); color:var(--on-teal);
      box-shadow:0 8px 20px -6px var(--glow-teal);
    }
    ...
    .filter-bar{
      ...
      margin:18px 0 22px; padding-bottom:12px; border-bottom:1px solid var(--line-2); flex-wrap:wrap; gap:12px;
    }
    ...
    .card-badge-rating{
      ...
      color:var(--gold); border:1px solid rgba(224,173,69,0.3); z-index:2;
    }
    .card-badge-eps{
    ...
    .load-more-area{
      margin-top:36px; text-align:center; display:flex; justify-content:center; gap:14px; align-items:center;
    }
...
<body>
  ...
  <div class="shell">
    <!-- Topbar -->
    <header class="topbar">
      <div class="brand" onclick="loadCatalog('1', 1)">
      ...
      <div class="top-actions">
        ...
        <button class="btn teal" onclick="downloadAllVisiblePosters()">
          🖼 ទាញយក Poster ទាំងអស់ (<span id="visiblePostersCount">0</span>)
        </button>
    ...
    <section class="hero">
      ...
      <div class="omni">
        ...
        <input type="text" id="searchInput" placeholder="ស្វែងរកចំណងជើងរឿងភាគ 8movie (វាយឈ្មោះ ឬពាក្យគន្លឹះ រួចចុច Enter)..." onkeydown="if(event.key==='Enter') doSearch(1)">
        <button class="btn primary" onclick="doSearch(1)">🔍 ស្វែងរក</button>
      </div>
    ...
    <div class="filter-bar">
      <div class="section-title">
        ...
        <span class="badge" id="pageBadge" style="display:none;">ទំព័រ 1</span>
      </div>
      <div class="filter-actions">
        <button class="btn teal" onclick="downloadAllVisiblePosters()">
          🖼 ទាញយក Poster ទាំងអស់
        </button>
    ...
    <div class="load-more-area" id="loadMoreArea">
      <button class="btn primary" style="padding:12px 28px; font-size:15px;" id="loadMoreBtn" onclick="loadNextPage()">
        ➕ បង្ហាញរឿង និង Poster បន្ថែមទៀត (ទំព័របន្ទាប់)
      </button>
    </div>
  </div>
    ...
    <div class="modal-card">
      ...
      <div class="modal-grid">
        <div class="modal-poster-wrap">
          <img id="modalPoster" src="" alt="Poster" onerror="this.src='/logo.png'">
  ...
  <script>
    ...
    function toast(msg, type='info'){
      ...
      t.className = 'toast';
      t.innerHTML = `<span>${type === 'success' ? '✅' : type === 'bad' ? '⚠️' : '🔔'}</span> <span>${msg}</span>`;
      box.appendChild(t);
      setTimeout(() => t.remove(), 4000);
    }

    // High performance image resolver: uses server proxy with disk cache and local fallback
    function getPosterImgUrl(rawUrl){
    ...
    function renderCategories(){
      ...
      row.innerHTML = CATEGORIES.map(c => `
        <button class="chip ${c.id === currentCat && !isSearchMode ? 'active' : ''}" onclick="selectCategory('${c.id}', '${c.name_km}')">
          ${c.name_km}
        </button>
      `).join('');
    }
    ...
    function selectCategory(catId, catName){
      ...
      loadCatalog(catId, 1, catName);
    }

    async function loadCatalog(catId, page=1, catName=null){
      currentCat = catId;
      currentPage = page;
      isSearchMode = false;
      renderCategories();
      ...
      const displayName = catName || (matchedCat ? matchedCat.name_km : '🎬 រឿងភាគ 8Movie');
      titleEl.innerText = displayName;

      const pageBadge = document.getElementById('pageBadge');
      pageBadge.style.display = 'inline-flex';
      pageBadge.innerText = `ទំព័រ ${page}`;

      const grid = document.getElementById('dramasGrid');
      if(page === 1){
        loadedDramas = [];
        grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:60px; color:var(--muted);">កំពុងទាញទិន្នន័យ និង Poster ពីរឿងភាគ 8movie.com...</div>';
      }

      try {
        const res = await fetch(`/api/catalog?cat=${catId}&page=${page}`);
        ...
        if(page === 1){
          ...
          // Append
          const existingIds = new Set(loadedDramas.map(d => d.id));
          items.forEach(it => {
            if(!existingIds.has(it.id)) loadedDramas.push(it);
          });
        }
        renderCards(loadedDramas);

        const moreBtn = document.getElementById('loadMoreBtn');
        if(items.length < 15){
          moreBtn.innerText = 'អស់រឿងភាគសម្រាប់បង្ហាញហើយ';
          moreBtn.disabled = true;
        } else {
          moreBtn.innerText = `➕ បង្ហាញរឿង និង Poster បន្ថែមទៀត (ទំព័រ ${currentPage + 1})`;
        ...
        if(page === 1){
          ...
          toast('មិនអាចទាញទំព័របន្ទាប់បានទេ: ' + e.message, 'bad');
        }
      }
    }
    ...
    async function doSearch(page=1){
      ...
      if(!q) return selectCategory(currentCat);
      ...
      document.getElementById('currentFeedTitle').innerText = `🔍 ស្វែងរក: "${q}"`;
      const pageBadge = document.getElementById('pageBadge');
      pageBadge.style.display = 'inline-flex';
      pageBadge.innerText = `ទំព័រ ${page}`;
      ...
      try {
        ...
        if(items.length < 15){
          ...
          moreBtn.innerText = `➕ បង្ហាញលទ្ធផលបន្ថែមទៀត (ទំព័រ ${currentPage + 1})`;
          moreBtn.disabled = false;
        }
      } catch(e){
        if(page === 1){
    ...
    function loadNextPage(){
      if(isSearchMode){
        ...
        loadCatalog(currentCat, currentPage + 1);
      }
    }
    ...
    function renderCards(items){
      ...
      document.getElementById('visiblePostersCount').innerText = `${items.length}`;
      ...
      grid.innerHTML = items.map(item => {
        const proxyUrl = getPosterImgUrl(item.poster);
        return `
          <div class="card">
            <div class="card-poster-box" onclick="openDrama('${item.id}')">
              <img class="card-poster"
                   src="${proxyUrl}"
                   alt="${item.title}"
                   loading="lazy"
                   onerror="this.onerror=null; this.src='${item.poster || '/logo.png'}';">
            ...
            <div class="card-content">
              ...
              <div class="card-actions">
                ...
                <button class="card-btn poster-btn" title="ទាញយក Poster HD នៃរឿងនេះ" onclick="downloadPosterDirect('${item.id}', '${escapeAttr(item.title)}', '${escapeAttr(item.title_km)}', '${item.poster}')">🖼 Poster</button>
                <button class="card-btn" title="ទាញយកគ្រប់ភាគទាំងអស់" onclick="quickDownloadAll('${item.id}', '${escapeAttr(item.title)}', '${escapeAttr(item.title_km)}')">⬇</button>
              </div>
            </div>
          </div>
        `;
    ...
    async function openDrama(id){
      ...
      try {
        ...
        document.getElementById('modalPoster').src = getPosterImgUrl(data.poster);
    ...
    async function downloadPosterDirect(id, title, title_km, posterUrl){
      toast('កំពុងបន្ថែម Poster ទៅក្នុងបញ្ជីទាញយក...');
      try {
    ...
    // 1-click Download All Posters currently shown on page
    async function downloadAllVisiblePosters(){
      if(!loadedDramas.length){
        return toast('មិនទាន់មាន Poster សម្រាប់ទាញយកឡើយ!');
      }
      toast(`កំពុងបន្ថែម Poster ទាំងអស់ចំនួន ${loadedDramas.length} រឿង...`);
      try {
        const res = await fetch('/api/download/posters_batch', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            dramas: loadedDramas
        ...
        const data = await res.json();
        toast(`បានបន្ថែម ${data.count} Posters ទៅក្នុងបញ្ជីទាញយកដោយជោគជ័យ!`, 'success');
        toggleDrawer(true);
      } catch(e){
        toast('មានបញ្ហាក្នុងការទាញយក Posters: ' + e.message, 'bad');
      }
    }
    ...
    async function pollDownloads(){
      try {
        ...
        listEl.innerHTML = data.tasks.slice(0, 40).map(t => `
          <div class="dl-item">
            <div style="display:flex; justify-content:space-between; font-size:13px; font-weight:700;">
              ...
              <span style="color:${t.status === 'completed' ? 'var(--good)' : t.status === 'downloading' ? 'var(--accent)' : 'var(--muted)'};">
                ${t.status === 'completed' ? 'រួចរាល់' : t.status === 'downloading' ? (t.speed || 'កំពុងទាញ...') : t.status}
              </span>
            </div>
          ...
          listEl.innerHTML = data.dramas.map(d => `
            <div style="background:var(--surface-2); border:1px solid var(--line); border-radius:14px; padding:14px; display:flex; justify-content:space-between; align-items:center;">
              <div>
                ...
                <div style="font-size:12px; color:var(--muted); margin-top:4px;">${d.episodes_count} ភាគបានទាញយក ${d.has_poster ? '· មាន Poster' : ''}</div>
              </div>
    ...
    function refreshCurrentFeed(){
      if(isSearchMode) doSearch(currentPage);
      else loadCatalog(currentCat, currentPage);
    }
...
print("Updated index.html with poster proxy, download-all-posters, and pagination.")
