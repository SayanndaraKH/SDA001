...
# Internal filename: 'D:\\code\\Hongguo-App\\installer\\_stage\\app\\downloader.py'
...
DL_SEG_MIN = int(os.environ.get('HG_DL_SEG_MIN', str(3145728)))
...
def _dl_session():
    ...
    if s is None:
        s = requests.Session()
        ad = requests.adapters.HTTPAdapter(pool_connections=DL_SEGMENTS * 2, pool_maxsize=DL_SEGMENTS * 4, max_retries=0)
        s.mount('https://', ad)
        s.mount('http://', ad)
        _dltls.s = s
    return s
...
def _probe(url):
    ...
    try:
        with _dl_session().get(url, headers={'Range': 'bytes=0-0'}, stream=True, verify=False, timeout=30) as r:
...
def _download_segmented(url, tmp, total, nseg, on_progress):
    ...
    def dl(i, lo, hi):
        s = _dl_session()
        try:
            with s.get(url, headers={'Range': f'bytes={lo}-{hi}'}, stream=True, verify=False, timeout=60) as r:
        ...
        try:
            if use_seg and (not os.path.exists(tmp)) and (expected_size is None or expected_size >= DL_SEG_MIN):
                ...
                if range_ok and total >= DL_SEG_MIN:
                    nseg = min(DL_SEGMENTS, max(1, total // 1048576))
                    seg_used = True
            ...
            with _dl_session().get(url, stream=True, headers=headers, verify=False, timeout=60) as r:
                if r.status_code == 416:
                    if expected_size and done >= expected_size * 0.98:
                        os.replace(tmp, path)
                        return True
                    done = 0
                    headers = {}
                    r = _dl_session().get(url, stream=True, verify=False, timeout=60)
...
def write_metadata(folder, meta, eps):
    ...
    json.dump({**meta, 'episodes': eps}, open(os.path.join(folder, 'info.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
    genres = ''.join((f'<genre>{g}</genre>' for g in meta.get('category') or []))
    actors = ''.join((f"<actor><name>{c.get('演员', '')}</name><role>{c.get('角色', '')}</role></actor>" for c in (meta.get('celebrities') or [])[:15]))
    nfo = f"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<tvshow>\n  <title>{meta.get('title', '')}</title>\n  <plot>{meta.get('intro', '')}</plot>\n  <premiered>{time.strftime('%Y-%m-%d', time.localtime(meta.get('create_time') or 0))}</premiered>\n  <status>{meta.get('status', '')}</status>\n  {genres}\n  {actors}\n</tvshow>\n"
...
class DownloadManager:
    ...
    def submit(self, series_id, rng='all', ep_covers=False):
        ...
        self.tasks[task_id] = {'task_id': task_id, 'series_id': series_id, 'state': '准备中', 'total': 0, 'done': 0, 'failed': 0, 'episodes': {}}
    ...
    def _dl_episode(self, task_id, folder, title, e, info, ep_covers):
        ...
        if os.path.exists(fn) and os.path.getsize(fn) > 0:
            ...
            if not info or not info.get('url'):
                ...
                ep['state'] = '下载中'
                ep['definition'] = info.get('definition')
                def progress(done, total):
                    ep['pct'] = done * 100 // total if total else 0
                def refetch():
                    u = self.get_video_urls([e['vid']], force=True).get(e['vid'], {})
                    return u.get('url')
                ok = download_one(info['url'], fn, expected_size=info.get('size'), refetch=refetch, on_progress=progress)
                if ok:
                    ep['state'] = '完成'
                    ep['pct'] = 100
                    t['done'] += 1
