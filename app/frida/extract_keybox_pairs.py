...
# Internal filename: 'D:\\code\\Hongguo-App\\installer\\_stage\\app\\frida\\extract_keybox_pairs.py'
...
ENTRY_PREFIX = b'\x01\r\x01\x00\x00\x00'
...
def classify_value(value: bytes) -> str:
    if value.endswith(b'\xeb\xeb'):
        ...
        if value[:8] != ZERO8 and value[8:] == ZERO8:
                return 'iv'
        if value != b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' and len(set(value)) >= 10 and (value.count(0) <= 3):
                    return 'key'
        return 'other'
def extract_mkid_spade(mm: mmap.mmap, back_window: int=8192) -> dict[str, set[str]]:
    ...
    while True:
        ...
        if off < 0:
            ...
            kids = re.findall(b'"mKid":"([0-9a-f]{32})"', window)
            if kids:
...
def iter_keybox_entries(mm: mmap.mmap):
    ...
    while off >= 0:
        if off + 32 <= len(mm) and mm[off + 8:off + 16] == ZERO8:
                ...
                yield {'offset': off, 'tag': bytes(mm[off + 6:off + 8]).hex(), 'value': value, 'kind': classify_value(value)}
...
def extract_key_votes(entries: list[dict], lookahead_bytes: int, iv_window: int):
    ...
    key_offsets = defaultdict(set)
    iv_candidates = defaultdict(Counter)
...
def build_report(dump_path: Path, lookahead_bytes: int, iv_window: int) -> dict:
    with dump_path.open('rb') as f:
        ...
        key_votes, key_offsets, iv_candidates = extract_key_votes(entries, lookahead_bytes=lookahead_bytes, iv_window=iv_window)
    joined = []
    ...
    for kid in all_kids:
        ...
        for key_hex, count in key_votes.get(kid, Counter()).most_common():
            keys.append({'key': key_hex, 'votes': count, 'offsets': [hex(x) for x in key_offsets[kid, key_hex]]})
        ivc = iv_candidates.get(kid, Counter())
        base_iv64 = None
        if ivc:
            pref = Counter((iv[:12] for iv in ivc)).most_common(1)[0][0]
            base_iv64 = min((iv for iv in ivc if iv[:12] == pref))
        joined.append({'kid': kid, 'spade_a': spade_values, 'spade_bytes': [decoded_spade_hex(x) for x in spade_values], 'keys': keys, 'base_iv64': base_iv64, 'iv8_candidates': [{'iv8': iv, 'votes': count} for iv, count in ivc.most_common(16)]})
    return {'dump': str(dump_path), 'entry_counts': dict(Counter((e['kind'] for e in entries))), 'mspade_kids': len(spades), 'keybox_kids': len(key_votes), 'joined': joined}
def main() -> None:
    ...
    report = build_report(Path(args.dump), lookahead_bytes=args.lookahead_bytes, iv_window=args.iv_window)
    ...
    for item in report['joined']:
        if item['keys']:
            key = item['keys'][0]
            print(f"kid={item['kid']} key={key['key']} votes={key['votes']} spade_count={len(item['spade_a'])}")
