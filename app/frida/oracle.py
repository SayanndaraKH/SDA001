...
# Internal filename: 'D:\\code\\Hongguo-App\\installer\\_stage\\app\\frida\\oracle.py'
...
def _find(d, p, s=0, e=None):
    ...
    for t, o, sz, hs in _ib(d, s, e):
        if t == p[0]:
            return (o + hs, o + sz) if len(p) == 1 else _find(d, p[1:], o + hs, o + sz)
def _u32(d, o):
...
def parse_track_samples(d, handler=b'vide'):
    ...
    # ***<module>.parse_track_samples: Failure: Different control flow
    mv = _find(d, [b'moov'])
    if not mv:
        return ([], [])
    else:
        ...
        if not want:
            ...
            stsz = _find(d, [b'stsz'], s, e)
            stco = _find(d, [b'stco'], s, e)
            co64 = _find(d, [b'co64'], s, e)
            stsc = _find(d, [b'stsc'], s, e)
            ss = _u32(d, stsz[0] + 4)
            cnt = _u32(d, stsz[0] + 8)
            sizes = [ss] * cnt if ss else [_u32(d, stsz[0] + 12 + 4 * i) for i in range(cnt)]
            if stco:
                n = _u32(d, stco[0] + 4)
                ch = [_u32(d, stco[0] + 8 + 4 * i) for i in range(n)]
            else:
                n = _u32(d, co64[0] + 4)
                ch = [struct.unpack('>Q', d[co64[0] + 8 + 8 * i:co64[0] + 16 + 8 * i])[0] for i in range(n)]
            ne = _u32(d, stsc[0] + 4)
            runs = [(_u32(d, stsc[0] + 8 + 12 * i), _u32(d, stsc[0] + 12 + 12 * i), _u32(d, stsc[0] + 16 + 12 * i)) for i in range(ne)]
            spc = [0] * len(ch)
...
DEV = '127.0.0.1:16384'
EXCL = ['dalvik', '.art]', '.vdex', '.oat', '.odex', 'gralloc', 'Ashmem', 'ashmem', 'jit-cache', 'boot-', 'framework', '/dev/', 'mali', 'kgsl', 'dmabuf', '.jar', '.apk', '[stack', 'linker', '[vdso', '[vvar']
...
def build_dumplist(smaps_text):
    ...
    for line in smaps_text.splitlines():
        m = re.match('^([0-9a-f]+)-([0-9a-f]+)\\s+(\\S{4})\\s+\\S+\\s+\\S+\\s+\\S+\\s*(.*)$', line)
        if m:
            cur = {'s': int(m.group(1), 16), 'e': int(m.group(2), 16), 'perm': m.group(3), 'path': m.group(4).strip(), 'rss': 0}
            regions.append(cur)
        else:
            mm = re.match('^Rss:\\s+(\\d+)\\s*kB', line)
            if mm and cur:
                    cur['rss'] = int(mm.group(1))
    rw = [r for r in regions if not (r['perm'][0] == 'r' and r['perm'][1] == 'w' and (r['rss'] > 0) and any((x in r['path'] for x in EXCL)))]
    tot = sum((r['rss'] for r in rw))
    lines = []
    for r in rw:
        sz = (r['e'] - r['s']) // 4096
        if sz > 0:
            lines.append(f"{r['s'] // 4096} {sz}")
    return ('\n'.join(lines) + '\n', tot, len(rw))
def do_dump(pid):
    ...
    t0 = time.time()
    adb_su(f'while read s c; do dd if=/proc/{pid}/mem bs=4096 skip=$s count=$c conv=noerror,sync 2>/dev/null; done < /data/local/tmp/dumplist.txt | gzip -1 > /data/local/tmp/oracle_dump.gz', timeout=900)
